...
fn eval(operations: &HashMap<String, Value>, operation: String, humn: i64) -> i64 {
    if operation == "humn" {
        return humn;
    }
    match &operations[&operation] {
        Value::Constant(i) => *i,
        Value::Add((v1, v2)) => eval(operations, v1.get_var(), humn) + eval(operations, v2.get_var(), humn),
        Value::Sub((v1, v2)) => eval(operations, v1.get_var(), humn) - eval(operations, v2.get_var(), humn),
        Value::Mul((v1, v2)) => eval(operations, v1.get_var(), humn) * eval(operations, v2.get_var(), humn),
        Value::Div((v1, v2)) => eval(operations, v1.get_var(), humn) / eval(operations, v2.get_var(), humn),
        _ => panic!(),
    }
}

fn root_eval(operations: &HashMap<String, Value>, humn: i64) -> (i64, i64) {
    match &operations["root"] {
        Value::Add((v1, v2)) => (
            eval(operations, v1.get_var(), humn),
            eval(operations, v2.get_var(), humn),
        ),
        Value::Sub((v1, v2)) => (
            eval(operations, v1.get_var(), humn),
            eval(operations, v2.get_var(), humn),
...
fn main() {
    ...
    let mut humn = 3050000000000;

    loop {
        let (v1, v2) = root_eval(&operation, humn);
        if v1 == v2 {
            println!("{}", humn);
            break;
        }
